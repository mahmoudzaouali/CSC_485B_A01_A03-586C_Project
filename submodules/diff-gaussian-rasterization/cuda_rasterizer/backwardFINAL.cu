...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs)
{
	...
	dL_dsh[0] = dRGBdsh0 * dL_dRGB;
		...
		dL_dsh[1] = (deg > 0) * dRGBdsh1 * dL_dRGB; //Could also remove deg > 0 multiplication, because these values are initialised to 0 if they don't exist?
		dL_dsh[2] = (deg > 0) * dRGBdsh2 * dL_dRGB;
		dL_dsh[3] = (deg > 0) * dRGBdsh3 * dL_dRGB;
		...
		dRGBdz = (deg > 0) ? SH_C1 * sh[2] : empty;
			...
			float dRGBdsh4 = deg > 1 ? SH_C2[0] * xy : 0;
			float dRGBdsh5 = deg > 1 ? SH_C2[1] * yz : 0;
			float dRGBdsh6 = deg > 1 ? SH_C2[2] * (2.f * zz - xx - yy) : 0;
			float dRGBdsh7 = deg > 1 ? SH_C2[3] * xz : 0;
			float dRGBdsh8 = deg > 1 ? SH_C2[4] * (xx - yy) : 0;
	
			dL_dsh[4] = (deg > 1) * dRGBdsh4 * dL_dRGB;
			dL_dsh[5] = (deg > 1) * dRGBdsh5 * dL_dRGB;
			dL_dsh[6] = (deg > 1) * dRGBdsh6 * dL_dRGB;
			dL_dsh[7] = (deg > 1) * dRGBdsh7 * dL_dRGB;
			dL_dsh[8] = (deg > 1) * dRGBdsh8 * dL_dRGB;

			dRGBdx += (deg > 1) * SH_C2[0] * y * sh[4] + SH_C2[2] * 2.f * -x * sh[6] + SH_C2[3] * z * sh[7] + SH_C2[4] * 2.f * x * sh[8];
			dRGBdy += (deg > 1) * SH_C2[0] * x * sh[4] + SH_C2[1] * z * sh[5] + SH_C2[2] * 2.f * -y * sh[6] + SH_C2[4] * 2.f * -y * sh[8];
			dRGBdz += (deg > 1) * SH_C2[1] * y * sh[5] + SH_C2[2] * 2.f * 2.f * z * sh[6] + SH_C2[3] * x * sh[7];
